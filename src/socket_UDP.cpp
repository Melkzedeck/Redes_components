...
void Usock::Bind(const Adress& add){
...
ssize_t Usock::recv(Adress& add, std::string& str){
    char msg[tam_max];
	struct sockaddr_storage addrSrg;
    struct sockaddr* aux = (sockaddr*) &addrSrg;
    socklen_t len_aux = sizeof(addrSrg);
	ssize_t cnt;
    cnt = recvfrom(socket_, msg, tam_max, 0, aux, &len_aux);
    add = Adress(aux);
